...
def plot_confusion_matrix(cm, class_names, save_path, title='Confusion Matrix', show_labels=True):
...
def test_model():
    ...
    # Полная матрица
    # plot_confusion_matrix(cm, class_names, save_path=cm_all, title='Матрица неточностей (все классы)', show_labels=False)

    # Top-20 классов с наибольшими ошибками
    top_cm, top_class_names = get_top_confused_classes(cm, class_names, top_k=20)
    plot_confusion_matrix(top_cm, top_class_names, save_path=cm_top20, title='Матрица неточностей (топ-20 классов)', show_labels=True)
